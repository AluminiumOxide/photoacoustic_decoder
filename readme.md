...
- 传入**p0** [0,1] 变量**label_img**=tensor(1,1,256,256) 
- 自动设置**us**  变量**matrix_us** = tensor(1,1,256,256)
    - label_img下，小于一定值后视为边界置0，大于一定值视为组织置1，
- 循环迭代
- 输出**ua**[0,1] 变量**net_output**=tensor(1,1,256,256) 
- 函数fai,p0=using_mcx(ua,us)

### using_mcx

- 整理ua 至 ua_tune ndarray(1,256,256,50) 
    - **ua_tune缩放至0~0.1**(人体组织常见范围)
- 整理us 至 us_tune ndarray(1,256,256,50) 
- 合并得到(2,256,256,50) 作为vol输入
    - mcxtry

- 返回fai ndarray(256,256)未进行归一化,且未切割边界
    - 先剪切边界置0
    - 再将数据归一化[0,1]
- p0 = fai*ua 数据大概在[0,1] 注: ua没有缩放，还是**0~1**
- 取log再归一化
...
- 不迭代边界(掐边理论没问题，而且效果还好，但是不优雅)、
- 边界和组织分别迭代(很容易炸) 、
- 一起迭代(选择了一种写论文最能凑字的方式)，效果在线，但是比第一种慢，不过确实是最优雅的



## pmcx生活质量调整

喜报！pmcx不用专门转换成z,x,y的矩阵！允许[?,x,y,z]且z=1的操作！！！

同时，pmcx的参数和mcxlab中的参数一致
...
这些参数的的索引是从1开始的(和python不一样)！就这里的几个索引值不一样，矩阵该传传
...
由于周围空气散射和组织不同，需要传入uaus的103格式数据
